 ...
 */
 ...
 define('JGB_EQMAT_URI_GET_OBJ_URL','get-dosf-url');
...
class Jgb_EqMat_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->plus_options = get_option(JGB_EQMAT_OPT_NM_PLUS_OPTIONS);
	}
	...
	public function enqueue_scripts() {
		...
		$dosfData['urlGetDosfURL'] = rest_url( '/'. JGB_EQMAT_APIREST_BASE_ROUTE .JGB_EQMAT_URI_GET_OBJ_URL . '/' );
		wp_localize_script(
			$this->plugin_name,
			'dosfDt',
			$dosfData
		...
		if( $valueToSearch !== false && !is_null($valueToSearch)){
			...
			if(is_array($res) && count($res)>0){
				...
				if( $this->plus_options['frontend-specific-match-search'] && $useSerialNmbCriterial ){
					$status = strtolower( Jgb_EqMat_Admin::get_dosf_status($res[0]->emision) );
					?>
